...
def process_forcing_for_correction (source, var, mit_grid_dir, out_file, in_dir=None, start_year=1979, end_year=None, monthly_clim=False):
    ...
    if source == 'ERA5':
        ...
        if in_dir is None:
            # Path on BAS servers
            in_dir = '/data/oceans_input/raw_input_data/CESM/LENS/daily/'
        file_head = 'PIctrl_'
        gtype = ['t', 't', 't', 't', 't']
        per_day = 4
    else:
...
def convert_pre_industrial_data(var):
    # initialise to ensemble number 1 and set the first year to 1770. The script runs for 180 years so the period created runs from 1770 to 1950
    ens = 1
    real_year = 1920
    for century in range(4,21):

        c = str(century).zfill(2)
        ...
        if var in ['FLDS', 'FSDS']:
            ...
            years = 100
            if century != 21:
                file_path="/data/oceans_input/raw_input_data/CESM/LENS/"+freq+"/"+var+"/"+"b.e11.B1850C5CN.f09_g16.005.cam.h0."+var+"."+c+"0001-"+c+"9912.nc"
            else:
                years = 101
                file_path="/data/oceans_input/raw_input_data/CESM/LENS/"+freq+"/"+var+"/"+"b.e11.B1850C5CN.f09_g16.005.cam.h0."+var+".210001-220012.nc"
        else:
            freq = 'daily'
            i_per_year = 365
            years = 100
            if century == 4:
                years = 98
                file_path="/data/oceans_input/raw_input_data/CESM/LENS/"+freq+"/"+var+"/"+"b.e11.B1850C5CN.f09_g16.005.cam.h1."+var+".04020101-04991231.nc"
            elif century == 21:
                years = 101
                file_path="/data/oceans_input/raw_input_data/CESM/LENS/"+freq+"/"+var+"/"+"b.e11.B1850C5CN.f09_g16.005.cam.h1."+var+".21000101-22001231.nc"
            else:
                file_path="/data/oceans_input/raw_input_data/CESM/LENS/"+freq+"/"+var+"/"+"b.e11.B1850C5CN.f09_g16.005.cam.h1."+var+"."+c+"000101-"+c+"991231.nc"

        for year in range(0,years-1):
            print(('Processing ' + str(100*century+year)))
            # Read data
            data = read_netcdf(file_path, var, t_start=(year)*i_per_year, t_end=(year+1)*i_per_year)
            # Unit conversions
            if var in ['FLDS', 'FSDS']:
                # Swap sign
                data *= -1
                ...
                data = data/(1.0 + data)
            # Write data
            out_file = '/data/oceans_input/processed_input_data/CESM/PIctrl/'
            if real_year > 2100:
                ens = ens + 1
                real_year = 1920
            ens_str = str(ens).zfill(2)
            write_binary(data, out_file+'PIctrl_ens' + ens_str + '_' + var + '_' + str(real_year))
