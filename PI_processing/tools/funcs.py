...
def ttest_pval(input_data):
    from scipy.stats import ttest_ind
    """ function calculates the difference between the trend function and the CTRL"""
    data = [input.trend.values * 100 for input in input_data]
    pvalue_total = []
    CTRL = data[-1]
    for i in range(3):
        DATA = data[i]
        pvalue = np.empty_like(DATA[0,...])
        for lat in range(np.shape(DATA)[1]):
            for lon in range(np.shape(DATA)[2]):
                [_, pvalue[lat, lon]] = ttest_ind(DATA[:, lat, lon], CTRL[:, lat, lon])
