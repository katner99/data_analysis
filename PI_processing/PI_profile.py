...
def plot_period(ax, period, var):
    ...
    filepaths = [
        output_path + "average_" + exp + "_" + period + ".nc" for exp in experiments
    ]
    ...
    profiles = [
        create_profile(data, var, grid, lat_range, lon_range, timeseries=False)
...
def main():
    ...
    exp = ["CTRL", "LENS", "TEMP", "WIND"]
    fig, axs = plt.subplots(
        nrows=1, ncols=4, gridspec_kw={"hspace": 0.05, "wspace": 0.05}
    )
    # axs = axs.flatten()
    var = "SALT"
    for i in range(4):
        plot_period(axs[i], exp[i], var, experiment_names[i])
